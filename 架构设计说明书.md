...
系统采用“飞书原生交互面 + OpenClaw Agent 主体 + 外部确定性业务内核”的模式：
- 飞书内负责承载文档、妙记、消息、任务、日历、多维表格等数据源与结果触达。
- OpenClaw 负责理解用户意图、选择办公场景、调用项目工具并组织反馈。
- 飞书外的 Node.js 服务负责事件接入、确定性业务动作、事项归并、状态管理、评测与观测。
- Trigger.dev 负责后台工作流安排、定时任务、补偿、重试和可重复执行的对账流程，不作为产品主体。
...
本文档描述最终系统设计，不代表当前代码已全部实现。当前项目处于早期 MVP 阶段，应按阶段推进而不是收缩最终目标。

| 能力 | 当前状态 | 说明 |
| --- | --- | --- |
| OpenClaw Agent 主体 | 部分实现 | 已有 OpenClaw WebSocket Gateway 调用和 `agent-tools` 工具服务；飞书端自然语言入口仍需验证 |
| 会后事项抽取 | 已实现雏形 | 已支持从会议/妙记内容抽取 WorkItem，并写入事项中枢 |
| 飞书任务创建 | 已实现雏形 | 已有 lark-cli task 适配；需要真实负责人和真实会议验证 |
| Base 推进总表 | 已实现雏形 | 已有 Base 投影；字段和真实权限仍需验证 |
| 会后确认卡片 | 部分实现 | 已有卡片渲染和发送；卡片回调闭环未完成 |
| 事项对账与风险巡检 | 部分实现 | 已有 Trigger.dev task；真实周期运行和告警效果待验证 |
| 会前背景卡片 | 未实现完整链路 | 仅有渲染/发送函数，日历触发和知识检索未接线 |
| 多源知识接入 | 未实现完整链路 | 最终要求包含 Docs/Wiki/IM/Task/Mail 等来源，当前主要聚焦会议入口 |
| 评测与观测 | 部分实现 | 已有结构化日志和报告生成脚本；OpenTelemetry / Langfuse 作为后续演进 |
...
系统分为 **OpenClaw Agent 主体 + 6 层主架构 + 2 个横切能力**：
- Agent 主体：
  - OpenClaw Agent：负责意图理解、场景判断、工具选择、反馈组织。
- 主架构：
  - 触点层
  - 触发与接入层
  - 工作流编排层
...
- 已实现结构化 JSON 日志。
- 已实现基础评测报告生成。
- 尚未接入 OpenTelemetry 与 Langfuse；二者属于最终系统的生产化演进目标，不作为当前 MVP 已实现能力表述。
...
- OpenClaw 连接：WebSocket Gateway + device auth
- 业务工具桥接：`agent-tools` HTTP API，供 OpenClaw Skill 调用
- 观测：结构化日志；OpenTelemetry + Langfuse 作为后续演进
...
OpenClaw Agent
  → 理解用户意图与场景
  → 选择 FeishuAgent 工具
  → 调用 agent-tools HTTP API
  → Node.js 业务内核执行确定性动作
  → lark-cli / 飞书 API 读取或写回资源
  → OpenClaw 组织用户可读反馈
```
...
```text
domain.llm.callLLM
  -> domain.openclaw-client.callAgent
  -> OpenClawGatewayWsClient
  -> OpenClaw WebSocket Gateway agent RPC
```

OpenClaw HTTP `/v1/*` 和 DeepSeek fallback 不再作为当前主路径。`agent-tools` HTTP 服务仍保留，因为它是 OpenClaw Skill 调用本项目业务工具的桥接接口。
...
- OpenClaw 是 Agent 主体，Trigger.dev 是后台确定性工作流与补偿调度。
- LLM / Agent 调用使用 OpenClaw WebSocket Gateway，结构化输出使用 Zod 校验。
