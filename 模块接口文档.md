...
- `workflow` 作为编排中心，调用 `domain`、`integration`、`touchpoint`
- `domain` 可直接调用 `integration`（读取飞书数据供 LLM 使用）
...
### 职责
定义 Trigger.dev 工作流任务。接收 TriggerEvent，按流程编排调用 domain、integration、touchpoint。
